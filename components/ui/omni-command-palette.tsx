/*
 * Command palette. Sourced from the 21st.dev registry
 * (lovesickfromthe6ix/omni-command-palette), retinted to project tokens.
 *
 * Chosen over three cheaper palettes because it ships async multi-source search,
 * fuzzy highlighting, recents persistence and real keyboard/aria wiring (17 aria
 * attributes) rather than a styled input in a dialog.
 *
 * NOTE: it has no internal prefers-reduced-motion guard. Our globals.css kills all
 * animation under that media query, so it is covered globally rather than locally.
 */
"use client";
