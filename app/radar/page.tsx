      ...
      <div className="flex">
        ...
        <aside className="sticky top-0 hidden h-screen w-60 flex-col border-r border-black/8 p-5 md:flex">
          ...
          <nav className="mt-8 flex flex-col gap-1 text-sm">
            {NAV.map((item, i) => (
              <Link key={item.label} href={item.href} className={`rounded-lg px-3 py-2 transition-colors ${i === 0 ? "bg-grn/10 font-medium text-grn" : "text-[#6b6f78] hover:bg-black/5 hover:text-white"}`}>{item.label}</Link>
            ))}
          </nav>
          <div className="mt-auto rounded-xl border border-black/10 bg-black/5 p-4">
            ...
            <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/10"><div className="h-full w-[62%] rounded-full bg-grn" /></div>
        ...
        <main className="min-w-0 flex-1 p-5 sm:p-8">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <button className="rounded-xl bg-grn px-4 py-2 text-sm font-semibold text-[#12220a] transition-colors hover:bg-[#1e40af]">Export CSV</button>
            </div>
          </div>
